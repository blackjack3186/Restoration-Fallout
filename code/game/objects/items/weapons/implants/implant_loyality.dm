...
/obj/item/weapon/implant/loyalty/get_data()
	var/dat = {"<b>Implant Specifications:</b><BR>
				<b>Name:</b> Vault-tec Employee Management Implant<BR>
...
/obj/item/weapon/implant/loyalty/implant(mob/target)
	if(..())
		...
		if((target.mind in ticker.mode.cult) || (target.mind in ticker.mode.blue_deity_prophets|ticker.mode.red_deity_prophets))
			...
			target << "<span class='notice'>You feel a surge of loyalty towards Vault-tec.</span>"
		return 1
	return 0
...
/obj/item/weapon/implant/loyalty/removed(mob/target, var/silent = 0)
	if(..())
		if(target.stat != DEAD && !silent)
			target << "<span class='boldnotice'>You feel a sense of liberation as Vault-tec's grip on your mind fades away.</span>"
		return 1
	return 0
